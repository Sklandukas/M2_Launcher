...
def find_focus(axis_service, capture_fn, beam_fn, axis_no, max_position, step_size, stop_event):
    ...
    while current <= max_position and not stop_event.is_set():
        ...
        t2 = time.time()
        print(f"Move to {current} took {t2 - t1:.2f} seconds")
        ...
        if img is None:
            ...
            continue
        t3 = time.time()
        print(f"Capture at {current} took {t3 - t2:.2f} seconds")

        t4 = time.time()
        res = beam_fn(img)
        area = res.Dx_mm * res.Dy_mm
        t5 = time.time()
        print(f"Beam analysis at {current} took {t5 - t4:.2f} seconds")
        ...
        if prev_area is not None and area > prev_area:
            ...
            inc_start = None
        t7 = time.time()
        print(f"Focus evaluation at {current} took {t7 - t6:.2f} seconds")
        prev_area = area
